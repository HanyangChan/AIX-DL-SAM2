...
def load_sam2_model(config_path, checkpoint_path, device="cuda"):
    ...
    # Check if config is a path or just a name
    if os.path.exists(config_path):
        config_name = os.path.basename(config_path)
        
        # Map standard configs to package paths
        # SAM 2.1 configs are in configs/sam2.1/
        # SAM 2 configs might be in configs/sam2/ or root, depending on version
        
        if config_name in ["sam2.1_hiera_t.yaml", "sam2.1_hiera_s.yaml", "sam2.1_hiera_b+.yaml", "sam2.1_hiera_l.yaml"]:
             print(f"Detected standard SAM 2.1 config {config_name}. Using package path.")
             # Try relative path
             relative_conf = f"configs/sam2.1/{config_name}"
             try:
                 sam2_model = build_sam2(relative_conf, checkpoint_path, device=device)
             except Exception as e:
                 print(f"Failed with relative path: {e}. Trying simple name.")
                 sam2_model = build_sam2(config_name, checkpoint_path, device=device)
        
        elif config_name in ["sam2_hiera_t.yaml", "sam2_hiera_s.yaml", "sam2_hiera_b+.yaml", "sam2_hiera_l.yaml"]:
             print(f"Detected standard SAM 2 config {config_name}. Using package path.")
             # Try relative path
             relative_conf = f"configs/sam2/{config_name}"
             try:
                 sam2_model = build_sam2(relative_conf, checkpoint_path, device=device)
             except Exception as e:
                 print(f"Failed with relative path: {e}. Trying simple name.")
             ...
             # For custom configs, build_sam2 might fail if passed as absolute path to config_name
             # But let's try.
