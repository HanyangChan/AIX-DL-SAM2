...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if os.path.exists(metadata_path) and os.path.exists(model_path):
            ...
            try:
                state_dict = torch.load(model_path, map_location=models["device"])
                
                # Check for DataParallel prefix 'module.'
                if list(state_dict.keys())[0].startswith('module.'):
                    state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
                
                # --- Dynamic Architecture Detection ---
                # Default to torchvision (EfficientNetV2-S) since that's what train_classifier.py uses
                use_timm = False
                model_arch = 'efficientnet_v2_s' 
                
                # Check for timm-specific keys
                if 'conv_stem.weight' in state_dict:
                    print("Detected 'conv_stem.weight' -> Likely a timm model.")
                    use_timm = True
                    stem_shape = state_dict['conv_stem.weight'].shape
                    if stem_shape[0] == 32:
                        print("Stem output 32 -> Likely EfficientNet-B0.")
                        model_arch = 'tf_efficientnet_b0'
                    elif stem_shape[0] == 24:
                        print("Stem output 24 -> Likely EfficientNetV2-S.")
                        model_arch = 'tf_efficientnetv2_s'
                
                # Check class count
                num_classes_ckpt = len(models["class_names"])
                if 'classifier.weight' in state_dict:
                    num_classes_ckpt = state_dict['classifier.weight'].shape[0]
                    print(f"Detected classifier output: {num_classes_ckpt} classes.")
                elif 'fc.weight' in state_dict:
                    num_classes_ckpt = state_dict['fc.weight'].shape[0]
                    print(f"Detected fc output: {num_classes_ckpt} classes.")
                
                if num_classes_ckpt != len(models['class_names']):
                    print(f"Warning: Metadata has {len(models['class_names'])} classes, but model has {num_classes_ckpt}.")
                
                # Initialize model
                if use_timm:
                    import timm
                    print(f"Initializing timm model {model_arch} with {num_classes_ckpt} classes...")
                    model = timm.create_model(model_arch, pretrained=False, num_classes=num_classes_ckpt)
                else:
                    print(f"Initializing torchvision model with {num_classes_ckpt} classes...")
                    # initialize_model uses torchvision.models.efficientnet_v2_s
                    model = initialize_model(num_classes_ckpt)

                # Load weights
                model.load_state_dict(state_dict)
            
            except ImportError:
                print("CRITICAL ERROR: 'timm' library is required to load this model but is not installed.")
                print("Please install it using: pip install timm")
                print("Falling back to uninitialized torchvision model (Predictions will be random!)")
                model = initialize_model(len(models["class_names"]))
            except Exception as e:
                print(f"Error checking/loading model: {e}")
                print("Falling back to uninitialized torchvision model.")
                model = initialize_model(len(models["class_names"]))
